...
for index, row in labels_df.iterrows():
    ...
    if os.path.exists(image_path):
        ...
        coords = np.array([row.iloc[1], row.iloc[2]], dtype=np.float32)
        x_coords.append(coords)

# Convert to numpy arrays and normalize
images = np.array(images).astype('float32') / 255.0
coords = np.array(x_coords)  # This will be shape (N, 2) where N is number of samples

# Create dataset without augmentation
tf_dataset = tf.data.Dataset.from_tensor_slices((images, coords))
...
def create_model(input_shape=(128, 128, 3)):
    ...
    # Output layer with 2 units for x,y coordinates
    coords = Dense(2, name='coords')(x)
    
    return Model(inputs=image_input, outputs=coords)
...
model.compile(
    optimizer=optimizer,
    loss='mse',  # Single loss for the combined coordinates
    metrics=['mae', 'mse']
)
